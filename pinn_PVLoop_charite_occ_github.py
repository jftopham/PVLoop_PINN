...
# File utilities
# ----------------------------
def exact_solution_graph(v,time, p0_i):
    w =  np.where( v.to_numpy()<=v0_final.detach().numpy(), 
                  sn_final.detach().numpy()*(np.log((v.to_numpy()-vd_final.detach().numpy())/(v0_final.detach().numpy()-vd_final.detach().numpy()))),
                  -sp_final.detach().numpy()*(np.log((vm_final.detach().numpy()-v.to_numpy())/(vm_final.detach().numpy()-v0_final.detach().numpy()))))
    z = (p0_i-w)*np.exp(-time.to_numpy()/tau_final.detach().numpy())  
    p = z+w
    return w, z, p
...
final_data.to_excel('X:/JFT/PINNs/charite_matlab_examples/IVC_occ/patients_data_occ/github/final_data.xlsx', index = False)